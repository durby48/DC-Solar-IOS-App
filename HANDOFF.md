...
- **Chase bank alerts** (Devon enabled 2026-07-27): business alerts email devon@dcsolarkc.com on "online deposit submitted" and "deposit > $1 posted" — they ride the same Gmail-filter pipeline (filter Chase's sender → DC-Notify label). Money-word subjects get a 💰 push. Bank alerts push to admins only.

## State / near-term TODO

0. **Notifications go-live (Devon, ~15 min total): work through `docs/NOTIFICATIONS_SETUP.md`** — APNs key, deploy `notify` + secret, Gmail labels/filters (Chase sender!), Apps Script trigger, optional payment webhook.
