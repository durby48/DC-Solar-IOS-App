...
- `twilio-voice-inbound` (v2, 2026-09-08 — **one person per number**): the number's Voice webhook. Looks the DIALED number up in **`voice_routes`** (`number_e164 → assigned_to`, an employees.email; seeded `+18167446473 → devonsd311@gmail.com`), logs the call as a `messages` row (channel call, direction in), identifies the caller like an inbound text, rings that one person's **app** (25 s) with `displayName` / ids as `<Parameter>`s so CallKit shows the customer's name, falls back to that person's own cell (`staff_profiles.cell_phone_e164`, 25 s, never a Twilio number — loop refused), then apologises, marks the row `no-answer` and pushes `📞 Missed call` to that person (to the admins when the number has no route). v1 rang every owner/operator with a voice identity; that is gone. A second number for someone else is one row, no code. Child legs report to `twilio-status` via `ParentCallSid`, so answered calls get their duration.
